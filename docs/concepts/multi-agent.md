...
seoTitle: "OpenClawのマルチエージェントルーティングの仕組みと設計ガイド"
read_when: "1つのゲートウェイプロセス内で、複数の分離されたエージェント（ワークスペース + 認証）を運用したい場合。"
description: "目標: 1つの実行中のゲートウェイ内で、複数の分離されたエージェント（個別のワークスペース + agentDir + セッション）、および複数のチャネルアカウント（例: 2つの WhatsApp アカウント）を運用すること。"
status: active
x-i18n:
