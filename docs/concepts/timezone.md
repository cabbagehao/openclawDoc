...
title: "タイムゾーン"
seoTitle: "OpenClawのタイムゾーン処理の考え方と設定上の注意点ガイド"
description: "OpenClaw はタイムスタンプを標準化するため、モデルは 単一の参照時刻を認識します。メッセージ エンベロープ (デフォルトではローカル)、例、ツール ペイロード (生のプロバイダー データ + 正規化されたフィールド)を確認できます。"
x-i18n:
