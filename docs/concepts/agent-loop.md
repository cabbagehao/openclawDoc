...
title: "エージェントループ"
seoTitle: "OpenClawのエージェントループの仕組みと応答処理フロー解説"
description: "エージェントループは、エージェントの完全な「実際の」実行プロセスです。インテーク（入力受付） → コンテキストのアセンブリ → モデル推論 → ツールの実行 → ストリーミング応答 → 永続化という一連の流れを指します。"
x-i18n:
