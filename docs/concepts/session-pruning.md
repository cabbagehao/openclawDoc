...
seoTitle: "OpenClawのセッション削減プルーニングの仕組みと保持設計ガイド"
summary: "セッションプルーニング: コンテキストの肥大化を抑えるためのツール実行結果のトリミング"
read_when:
description: "セッションプルーニングは、LLM を呼び出す直前に、メモリ上のコンテキストから 古いツールの実行結果 をトリミング（削減）する機能です。ディスク上のセッション履歴 (*.jsonl) を書き換えることは ありません。"
x-i18n:
