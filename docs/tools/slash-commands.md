...
seoTitle: "OpenClawスラッシュコマンドの使い方と設計ルールガイド"
description: "コマンドはゲートウェイによって処理されます。ほとんどのコマンドは、/ で始まる スタンドアロン メッセージとして送信する必要があります。ホスト専用 bash チャット コマンドは、! (エイリアスとして /bash を使用) を使用します。"
x-i18n:
