...
title: "Sub-Agents"
seoTitle: "OpenClawサブエージェント機能の使い方と運用設計ガイド"
description: "サブエージェントは、既存のエージェント実行から生成されたバックグラウンド エージェント実行です。これらは独自のセッション (agent::subagent:) で実行され、終了すると、結果を要求者のチャット チャネルに発表します。"
x-i18n:
