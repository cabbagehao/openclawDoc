...
title: "Plugins"
seoTitle: "OpenClawプラグインツールの使い方と拡張フロー解説ガイド"
description: "プラグインは、OpenClaw を拡張する単なる 小さなコード モジュールです。機能 (コマンド、ツール、ゲートウェイ RPC)。ほとんどの場合、構築されていない機能が必要な場合はプラグインを使用します。"
x-i18n:
...
(WhatsApp、Telegramなど)。チャネル構成は `channels.<id>` の下にあり、
