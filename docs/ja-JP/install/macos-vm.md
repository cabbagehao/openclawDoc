...
# macOS VM での OpenClaw (サンドボックス化)

## 推奨されるデフォルト (ほとんどのユーザー向け)

- **小規模な Linux VPS**: 常時稼働の Gateway と低コスト。 [VPS ホスティング](/vps) を参照してください。
- **専用ハードウェア** (Mac mini または Linux マシン): ブラウザ自動化のための完全な制御と**住宅用 IP** が必要な場合。 多くのサイトはデータセンターの IP をブロックするため、ローカルブラウジングの方がうまく機能することがよくあります。
- **ハイブリッド**: Gateway を安価な VPS に維持し、ブラウザ/UI の自動化が必要なときに Mac を**ノード**として接続します。 [ノード](/nodes) と [Gateway リモート](/gateway/remote) を参照してください。

macOS 専用の機能 (iMessage/BlueBubbles) が特に必要な場合、または日常の Mac からの厳密な分離が必要な場合に、macOS VM を使用します。
...
- 他のホスト型 Mac ベンダーも機能します。彼らの VM + SSH ドキュメントに従ってください
...
これは macOS で実行するキラー機能です。 [BlueBubbles](https://bluebubbles.app) を使用して iMessage を OpenClaw に追加します。
...
Gateway を再起動します。 これで、エージェントは iMessage を送受信できるようになりました。
...
- [VPS ホスティング](/vps)
- [ノード](/nodes)
- [Gateway リモート](/gateway/remote)
- [BlueBubbles チャネル](/channels/bluebubbles)
