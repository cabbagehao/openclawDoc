...
OpenClaw は、`openclaw.ai` から提供される 3 つのインストーラースクリプトを用意しています。
...
<Steps>
  ...
  <Step title="インストール後のタスク">
    可能であれば必要な bin ディレクトリをユーザー PATH に追加。アップグレードおよび git インストール時に `openclaw doctor --non-interactive` を実行（ベストエフォート）。
  </Step>
</Steps>
