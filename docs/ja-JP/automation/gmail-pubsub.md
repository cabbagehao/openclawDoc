...
- OpenClaw で hooks が有効になっていること（[Webhooks](/automation/webhook) を参照）。
...
ペイロード処理をさらにカスタマイズしたい場合は、`hooks.mappings` を追加するか、`~/.openclaw/hooks/transforms` 配下に JS/TS の transform モジュールを置いてください（[Webhooks](/automation/webhook) を参照）。
...
- `hooks.enabled=true` かつ `hooks.gmail.account` が設定されている場合、Gateway は起動時に `gog gmail watch serve` を開始し、watch を自動更新します。
...
- `--hook-url` は OpenClaw の `/hooks/gmail` を指します（マッピング済み。隔離実行とメインへの要約を行います）。
