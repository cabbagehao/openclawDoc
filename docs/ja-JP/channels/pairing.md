...
ノードは、`role: node` を使用して **デバイス** としてゲートウェイに接続します。ゲートウェイ
承認が必要なデバイスのペアリング要求を作成します。
...
2. ボットは 2 つのメッセージで応答します。1 つは指示メッセージ、もう 1 つは別の **セットアップ コード** メッセージ (Telegram で簡単にコピー/ペーストできます) です。
3. 携帯電話で、OpenClaw iOS アプリ → [設定] → [ゲートウェイ] を開きます。
4. セットアップコードを貼り付けて接続します。
5. テレグラムに戻ります: `/pair approve`

セットアップ コードは、base64 でエンコードされた JSON ペイロードで、以下が含まれます。

- `url`: ゲートウェイ WebSocket URL (`ws://...` または `wss://...`)
- `token`: 有効期間が短いペアリング トークン
...
- `pending.json` (有効期間は短く、保留中のリクエストは期限切れになります)
- `paired.json` (ペアリングされたデバイス + トークン)

### 注記- 従来の `node.pair.*` API (CLI: `openclaw nodes pending/approve`) は、

ゲートウェイが所有する別のペアリング ストア。 WS ノードでは引き続きデバイスのペアリングが必要です。
