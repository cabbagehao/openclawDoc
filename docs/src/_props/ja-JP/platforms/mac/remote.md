...
# リモート OpenClaw（macOS ⇄ リモート ホスト）

この構成では、macOS アプリを、別ホスト（デスクトップ / サーバー）で動作する OpenClaw ゲートウェイの完全なリモート コントローラーとして利用できます。これはアプリの **Remote over SSH**（remote run）機能です。ヘルスチェック、Voice Wake の転送、Web Chat を含むすべての機能が、*Settings → General* で設定した同じ SSH 構成を共有します。
