...
* ワンショット（`at`）ジョブは、transient error（rate limit、overloaded、network、server\_error）をバックオフ付きで最大 3 回までリトライします。permanent error は即座に無効化されます。詳しくは [Retry policy](/automation/cron-jobs#retry-policy) を参照してください。
