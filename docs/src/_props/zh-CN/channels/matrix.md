...
6. 从任何 Matrix 客户端（Element、Beeper 等；参见 [https://matrix.org/ecosystem/clients/）与机器人开始私信或邀请它加入房间。Beeper](https://matrix.org/ecosystem/clients/）与机器人开始私信或邀请它加入房间。Beeper) 需要 E2EE，因此请设置 `channels.matrix.encryption: true` 并验证设备。
