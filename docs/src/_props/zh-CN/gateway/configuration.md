...
* `agents.defaults.blockStreamingBreak`：`"text_end"` 或 `"message_end"`（默认：text\_end）。
...
pi-ai 内置的 `opencode` 提供商；从 [https://opencode.ai/auth](https://opencode.ai/auth) 设置 `OPENCODE_API_KEY`（或
