...
`sessionKey`는 *어떤 대화 버킷* 안에 있는지를 식별합니다(routing + isolation).
...
* `custom_message`: model context에 *들어가는* extension 주입 메시지(UI에서는 숨길 수 있음)
* `custom`: model context에 *들어가지 않는* extension state
