...
summary: "Vercel AI Gateway 설정(auth + model selection)"
read_when:
...
* OpenClaw 는 Gateway 의 `/v1/models` 카탈로그를 자동 발견하므로, `/models vercel-ai-gateway`
  에는 `vercel-ai-gateway/openai/gpt-5.4` 같은 최신 model ref 가 포함됩니다.
...
OpenClaw 는 Vercel Claude shorthand model ref 를 받아 런타임에 정규화합니다:
