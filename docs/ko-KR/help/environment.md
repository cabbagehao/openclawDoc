...
description: "OpenClaw가 환경 변수를 어떤 순서로 읽고 적용하는지, `env` 블록과 셸 import, 경로 관련 변수까지 한 번에 설명합니다."
read_when:
...
OpenClaw는 여러 소스에서 환경 변수를 불러옵니다. 기본 원칙은 **이미 존재하는 값을 절대 덮어쓰지 않는 것**입니다.

## 우선순위(높음 -> 낮음)

1. **프로세스 환경 변수**: Gateway 프로세스가 이미 부모 셸 또는 데몬으로부터 받은 값
...
config 파일 자체가 없으면 4단계는 건너뜁니다. 셸 import는 활성화되어 있다면 계속 실행됩니다.

## Config `env` 블록

인라인 환경 변수를 설정하는 동등한 방법이 두 가지 있습니다. 둘 다 기존 값을 덮어쓰지 않습니다.
...
- `OPENCLAW_SHELL=acp`: ACP 런타임 백엔드 프로세스를 생성할 때 설정됩니다. 예: `acpx`
- `OPENCLAW_SHELL=acp-client`: `openclaw acp client`가 ACP 브리지 프로세스를 생성할 때 설정됩니다.
- `OPENCLAW_SHELL=tui-local`: 로컬 TUI의 `!` 셸 명령에 설정됩니다.

이 값들은 사용자 설정 항목이 아니라 런타임 표식입니다. 셸 또는 프로필 로직에서 컨텍스트별 규칙을 적용할 때 활용할 수 있습니다.
...
OpenClaw는 환경 변수 기반 패턴을 두 가지 방식으로 지원합니다.
...
| Variable               | Purpose                                                                                                                                         |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | 모든 내부 경로 해석(`~/.openclaw/`, agent 디렉터리, sessions, credentials)에 사용할 홈 디렉터리를 재정의합니다. 전용 서비스 계정으로 OpenClaw를 실행할 때 유용합니다. |
...
| Variable             | Purpose                                                                                                                                                 |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_LOG_LEVEL` | 파일과 콘솔 모두의 로그 레벨을 재정의합니다. 예: `debug`, `trace`. config의 `logging.level` 및 `logging.consoleLevel`보다 우선합니다. 잘못된 값은 경고 후 무시됩니다. |

### `OPENCLAW_HOME`

`OPENCLAW_HOME`을 설정하면 시스템 홈 디렉터리(`$HOME` / `os.homedir()`) 대신 이 값을 모든 내부 경로 해석에 사용합니다. 이를 통해 headless 서비스 계정에서도 완전한 파일시스템 격리를 구성할 수 있습니다.
...
- [FAQ: env vars 및 .env 로드](/help/faq#env-vars-and-env-loading)
- [Models 개요](/concepts/models)
