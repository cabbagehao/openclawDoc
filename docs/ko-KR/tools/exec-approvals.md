...
description: "host exec 승인, allowlist, safe bins, 채팅 승인 전달과 node·Gateway 실행 가드레일을 설명합니다."
...
승인 기반 interpreter/runtime 실행은 의도적으로 보수적으로 동작합니다.

- 정확한 argv/cwd/env 컨텍스트는 항상 바인딩됩니다.
- 직접 실행하는 shell script와 직접 런타임 파일 형태는 best-effort로 하나의 구체적인 로컬 파일 스냅샷에 바인딩됩니다.
- OpenClaw가 interpreter/runtime 명령에 대해 **정확히 하나의 구체적인 로컬 파일**을 식별할 수 없으면, 예를 들어 package script, eval 형태, runtime별 loader chain, 모호한 multi-file 형태 같은 경우에는 semantic coverage를 가장하지 않고 승인 기반 실행을 거부합니다.
...
Discord와 Telegram은 채널별 설정을 통해 명시적인 exec approval client로도 동작할 수 있습니다.

- Discord: `channels.discord.execApprovals.*`
- Telegram: `channels.telegram.execApprovals.*`

이 클라이언트는 opt-in입니다. 어떤 채널에서 대화가 이루어졌다는 이유만으로, exec approvals가 켜져 있지 않은 채널을 OpenClaw가 자동으로 승인 surface로 취급하지는 않습니다.
...
- operator UI나 구성된 approval client가 아무도 요청을 받을 수 없으면 프롬프트는 `askFallback`으로 돌아갑니다.

Telegram은 기본적으로 approver DM(`target: "dm"`)을 사용합니다. 원하면 `channel` 또는 `both`로 바꿔, 원래의 Telegram chat/topic에도 승인 프롬프트가 나타나게 할 수 있습니다. Telegram forum topic에서는 approval prompt와 승인 후 후속 메시지 모두 topic을 유지합니다.
