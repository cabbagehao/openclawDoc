...
description: "node가 연결돼 있어도 camera, canvas, location, `system.run` 등이 실패할 때 확인할 foreground, pairing, permissions, approvals 점검표입니다."
read_when:
  - "node는 연결돼 있지만 camera/canvas/screen/exec tools가 실패할 때"
...
status에는 node가 보이지만 node tools가 실패할 때 이 페이지를 사용하세요.
...
- node가 연결되어 있고 `node` role로 paired 상태
...
`NODE_BACKGROUND_UNAVAILABLE`가 보이면 node app을 foreground로 가져온 뒤 다시 시도하세요.
...
1. **Device pairing**: 이 node가 gateway에 연결할 수 있는가
2. **Exec approvals**: 이 node가 특정 shell command를 실행할 수 있는가
...
pairing이 없다면 먼저 node device를 승인하세요.
pairing은 정상인데 `system.run`이 실패한다면 exec approvals/allowlist를 고치세요.

## Common node error codes

- `NODE_BACKGROUND_UNAVAILABLE` → app이 background 상태. foreground로 전환하세요.
...
- `LOCATION_BACKGROUND_UNAVAILABLE` → app이 background인데 While Using만 있음
- `SYSTEM_RUN_DENIED: approval required` → exec 요청에 명시적 승인이 필요
- `SYSTEM_RUN_DENIED: allowlist miss` → allowlist mode에서 command가 차단됨
  Windows node hosts에서는 allowlist mode에서 `cmd.exe /c ...` 같은 shell-wrapper 형식이 ask flow로 승인되지 않는 한 allowlist miss로 처리됩니다.
