...
모든 카메라 접근은 **사용자가 제어하는 설정** 뒤에 있습니다.
...
- `camera.snap`
  ...
  - 응답 payload:
    ...
    - `base64: "<...>"`
    - `width`, `height`
  - Payload guard: 사진은 base64 payload가 5MB 아래가 되도록 다시 압축됩니다.
...
첨부 파일을 얻는 가장 쉬운 방법은 디코딩한 미디어를 temp file에 쓰고 `MEDIA:<path>`를 출력하는 CLI helper입니다.

예시:

```bash
openclaw nodes camera snap --node <id>               # 기본: front + back 둘 다(2개의 MEDIA 줄)
...
- `nodes camera snap`은 기본적으로 **양쪽 방향 모두** 를 캡처해 에이전트에 두 시점을 모두 제공합니다.
...
권한이 없으면 앱이 가능할 때 prompt를 띄우며, 거부되면 `camera.*` 요청이 `*_PERMISSION_REQUIRED` 오류로 실패합니다.
...
사진은 base64 payload가 5MB 아래가 되도록 다시 압축됩니다.
...
openclaw nodes camera clip --node <id> --duration-ms 3000      # MEDIA:<path> 출력 (legacy flag)
...
- `openclaw nodes camera snap`은 override하지 않으면 기본 `maxWidth=1600`을 사용합니다.
- macOS에서는 `camera.snap`이 warm-up / exposure settle 이후 `delayMs`(기본 2000ms)를 기다린 뒤 촬영합니다.
- 사진 payload는 base64가 5MB 아래가 되도록 다시 압축됩니다.

## 안전 및 실용적 제한

- 카메라와 마이크 접근은 일반적인 OS 권한 prompt를 띄우며(Info.plist usage string 필요), 사용자가 허용해야 합니다.
- oversized node payload(base64 오버헤드 + 메시지 제한)를 막기 위해 동영상 클립은 현재 `<= 60s`로 제한됩니다.
