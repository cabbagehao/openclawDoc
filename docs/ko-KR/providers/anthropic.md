...
description: "OpenClaw에서 API 키 또는 setup-token으로 Anthropic Claude 사용하기"
read_when:
...
프로바이더 header를 직접 재정의한다면 이를 유지하세요([/gateway/configuration](/gateway/configuration) 참고).
