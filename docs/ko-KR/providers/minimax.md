...
description: "OpenClaw에서 MiniMax M2.5 사용하기"
read_when:
...
## MiniMax M2.5와 MiniMax M2.5 Highspeed 비교
...
**적합한 경우:** API 키 없이 OAuth로 MiniMax Coding Plan을 빠르게 설정하고 싶을 때
...
### MiniMax M2.5를 폴백으로 사용하는 예시

**적합한 경우:** 가장 강한 최신 세대 모델을 primary로 유지하고, 실패 시 MiniMax M2.5로 폴백하고 싶을 때
...
### 선택 사항: Local via LM Studio (manual)
...
이는 보통 **MiniMax 프로바이더가 설정되지 않았음**을 의미합니다. 프로바이더 entry가 없고, MiniMax auth profile/env key도 발견되지 않은 상태입니다. 이 감지 로직 수정은 **2026.1.12**에 포함되어 있습니다(작성 시점 기준 미출시).
...
- `MINIMAX_API_KEY` 또는 MiniMax auth profile을 설정해 프로바이더가 주입되도록 만들기
