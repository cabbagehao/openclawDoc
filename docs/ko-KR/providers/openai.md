...
description: "OpenClaw에서 API 키 또는 Codex 구독으로 OpenAI 사용하기"
read_when:
...
**적합한 경우:** 직접 API에 접근하고 사용량 기반으로 과금할 때.
...
**적합한 경우:** API 키 대신 ChatGPT/Codex 구독 액세스를 사용할 때.
...
WebSocket 워밍업(`openaiWsWarmup: true`)도 활성화합니다.
...
OpenAI 문서는 워밍업을 선택 사항으로 설명합니다. OpenClaw는 WebSocket transport 사용 시
...
OpenAI API는 `service_tier=priority`를 통해 우선 처리을 노출합니다.
