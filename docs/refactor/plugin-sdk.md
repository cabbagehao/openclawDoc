...
seoTitle: "OpenClawのプラグインSDK改修の目的と変更範囲ガイド"
description: "目標: すべてのメッセージング コネクタは、1 つの安定した API を使用するプラグイン (バンドルまたは外部) です。src/** から直接インポートするプラグインはありません。すべての依存関係は SDK またはランタイムを経由します。"
x-i18n:
